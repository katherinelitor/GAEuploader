...
GAEuploader-$(VERSION)-mac.zip: $(SDK_MAC)
	...
	git archive master -- README LICENSE uploader contents | tar -C staging/mac/GAEuploader -xvf -
	tar -C staging/mac/GAEuploader/contents -xf $(SDK_MAC)
	cd staging/mac && zip --symlink -q -r -9 "../../$@" GAEuploader
	rm -rf staging/

GAEuploader-$(VERSION)-linux.zip: $(SDK_LINUX)
	rm -rf staging/linux
	mkdir -p staging/linux/GAEuploader
	git archive master -- README LICENSE uploader contents | tar -C staging/linux/GAEuploader -xvf -
	tar -C staging/linux/GAEuploader/contents -xf $(SDK_LINUX)
...
GAEuploader-$(VERSION)-windows.zip: $(SDK_WINDOWS)
	...
	git archive master -- README LICENSE contents uploader.cmd | tar -C staging/windows/GAEuploader -xvf -
	cp -a $(SDK_WINDOWS) staging/windows/GAEuploader/contents
